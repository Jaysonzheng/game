...
card_logic_value(Card) when Card < 1 -> -1;
card_logic_value(Card) when Card > 16#42 -> -1;
...
%% check cards type
%%
check_card_type(Cards, []) -> invalid_card;
...
is_missile(Cards) ->
	case length(Cards) =:= 2 andalso
		lists:nth(1, Cards) =:= 16#42 andalso 
		lists:nth(2, Cards) =:= 16#41 of
	true -> type_missile;
	false -> invalid_card
	end.
...
%% 单连判断
is_single_line(Cards) when length(Cards) < 2 -> invalid_card;
...
%% 连对判断
is_double_line(Cards) when length(Cards) == 0 orelse length(Cards) rem 2 /= 0 -> invalid_card;
...
%% 三连判断
is_three_line(Cards) when length(Cards) < 6 orelse length(Cards) rem 3 /= 0 -> invalid_card;
is_three_line(Cards) ->
	[E|R] = Cards,
	Value = card_logic_value(E),
...
%	?DEBUG("Value = ~p, PreValue = ~p~n", [Value, PreValue]),
...
test() ->
	...
	?assert(is_three_take_single([16#34, 16#24, 16#23, 16#03]) == invalid_card),
	...
	?assert(is_three_take_double([16#26, 16#15, 16#35, 16#24, 16#14]) == invalid_card),
	
	?assert(get_card_type([16#22, 16#32]) == type_double), 
	?assert(get_card_type([16#42, 16#41]) == type_missile),
	...
	?assert(get_card_type([16#2d, 16#1d, 16#2c, 16#1c]) == invalid_card),
	
	?assert(get_card_type([16#2d, 16#1d, 16#0d, 16#2c, 16#1c, 16#0c]) == three_line),
	?assert(get_card_type([16#2d, 16#1d, 16#0d, 16#2d, 16#1d, 16#0d, 16#2c, 16#1c, 16#0c]) == invalid_card),
	?assert(get_card_type([16#2d, 16#1d, 16#0d, 16#2c, 16#1c, 16#0c, 16#23, 16#25]) == three_line_take_single),
	?assert(get_card_type([16#2d, 16#1d, 16#0d, 16#2c, 16#1c, 16#0c, 16#25, 16#25]) == three_line_take_single),
	?assert(get_card_type([16#2d, 16#1d, 16#0d, 16#2c, 16#1c, 16#0c, 16#25, 16#15, 16#23, 16#13]) == three_line_take_double),

	?assert(get_card_type([16#2c, 16#1c, 16#0c, 16#2b, 16#2a]) == invalid_card),
	?assert(get_card_type([16#2c, 16#1b, 16#0b, 16#2a, 16#1a]) == invalid_card),
	?assert(get_card_type([16#53]) == invalid_card),
	?assert(get_card_type([16#43]) == invalid_card),
	
	%Value = get_card_type([16#53]),
	%Value = get_card_type([16#2d, 16#1d, 16#0d, 16#2c, 16#1c, 16#0c, 16#25, 16#15, 16#23, 16#13]),
